...
export function ShareBar({ title, url }: ShareBarProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-[10px] font-black uppercase tracking-[0.25em] text-zinc-400 mr-1">Share</span>
      {platforms.map((p) => (
        <a
          key={p.name}
          href={p.href}
      ...
      <button
        onClick={copyLink}
        title="Copy link"
        className="w-8 h-8 rounded-full border border-zinc-200 flex items-center justify-center text-zinc-400 hover:text-zinc-800 hover:border-zinc-400 transition-all duration-200"
      >
